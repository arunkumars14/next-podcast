...
const PodcastDetails = ({params: {podcastId}}: {params: {podcastId: Id<"podcasts">}}) => {
  ...
  const podcast: any = useQuery(api.podcasts.getPodcastById, {
    podcastId
  })
    ...
    <section className="flex w-full flex-col">
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-4 ">
          ...
          <p className="text-16 font-medium text-white-2">{podcast?.imagePrompt === "" ? "Creater used custom thumbnail" : podcast?.imagePrompt}</p>
        </div>
      </div>

      <section className="mt-8 flex-col flex gap-5">
        ...
        {similarPodcast && similarPodcast.length > 0 ? (
          ...
          <>
           <EmptyState title="No similar podcast found" buttonLink="/discover" buttonText="Discover more podcasts" />
          </>
        )}
