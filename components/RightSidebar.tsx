...
const RightSidebar = () => {
  ...
  return (
    <section className={cn("right_sidebar text-white-1 h-[calc(100vh-5px)] pb-2", {"h-[calc(100vh-128px)]" : audio?.audioUrl})}>
      <SignedIn>
        ...
        <div className="flex flex-col gap-6">
          {
            topPodcasters?.slice(0, 5).map((podcaster) => (
              <div className="flex cursor-pointer justify-between" key={podcaster?._id} onClick={() => router.push(`/profile/${podcaster?.clerkId}`)}>
